...
app.get("/users", async (req: Request, res: Response) => {
  try {
    ...
    // INSTANCIAR O userDatabase
    const userDatabase = new UserDatabase();
    const usersDB = await userDatabase.findUsers(q);

    // ESSA LÓGICA DO CÓDIGO VAI PARA O UserDatabase.ts COM ALGUMAS MODIFICAÇÕES
    // let usersDB
...
app.post("/users", async (req: Request, res: Response) => {
  try {
    ...
    // FOI PARA A LÓGICA DENTRO DE UserDatabase.ts
    ...
    //  NÃO PRECISA INSTANCIAR PORQUE JÁ FOI FEITO NA LINHA 110

    await userDatabase.craeteUser(newUserDB);
...
app.get("/accounts", async (req: Request, res: Response) => {
  try {
    ...
    const accountsDB = await accountDatabase.getAllAccounts();

    // LÓGICA VAI PARA O AccountDatabase
...
app.post("/accounts", async (req: Request, res: Response) => {
  try {
    ...
    await accountDatabase.craeteAccount(newAccountDB);
